...
var (
	mssrv    = flag.String("mssrv", "xa-lsr-jimweiw7", "ms sql server name")
	msdb     = flag.String("msdb", "Account_001_Kaikei", "ms sql server database name")
	msuser   = flag.String("msuser", "sa", "ms sql server user name")
	mspass   = flag.String("mspass", "xA123456", "ms sql server password")
...
func mssqlConnect() (db *sql.DB, stmtCount int, err error) {
	...
	fmt.Println("the param is", c)
	fmt.Println("connect to sql server successfully.")
